...
<div onClick="window.open('request3','self')">المبيعات الشهرية</div>
...
<div class="path1">التصنيفات<div class="path140">التصنيفات /ادارة التصنيفات </div><div class=""><?  $q=$_GET['q'];if($q==1){
echo"تم اضافة التصنيف بنجاح";}
?> </div></div>

<div onClick="window.open('c1','self')" class="path">  <button> اضافة تصنيف جديد  </button>
</div>
...
<th  class="text" width="10%">التسلسل</th>
<th  class="text" width="60%">اسم التصنيف</th>
...
$result333= mysql_query("SELECT * FROM cats ");
...
$result111 = mysql_query("SELECT * FROM cats limit $start , $end");
$n= mysql_num_rows($result111);
if ($n<1){?>
...
?> <tr>
<td> <span  class="bad"> <? echo $myrow111['id']; ?> </span></td>
<td>  <span  class="bad1">  <?  echo $myrow111['name_cat']; ?>  </span> </td>
<td>   <span  class="bad12"> <a href="edit_cat.php?id=<? echo $myrow111['id']; ?>"><div class="path123"><button>edit</button></div></a> </span>  
  <a href="delete_cat.php?id=<? echo $myrow111['id']; ?>"><div class="path1234"><button>delete</button></div></a> 
 </td>
</tr>
<?
}
...
<? 
		 ...
		 if($next<=$pagecount)
		 echo'<div class="path123456"><a href =cat.php?id='.$id. '&page='.$next.'><h style="font-size:2vmin;">NEXT</h></a></div>';
			...
			if($prev >0)
			 ...
			 echo'<div class="path12345"><a href=cat.php?id='.$id.'&page='.$prev.'><h style="font-size:2vmin;">PREVIUOS</a> </div>';
			 ?>
